  ...
  useEffect(() => {
    const initMap = () => {
      ...
      const mapInstance = new google.maps.Map(mapRef.current, {
        ...
        zoom: 12,
        styles: [
          { featureType: 'all', elementType: 'geometry.fill', stylers: [{ color: '#e8f4fd' }] },
          { featureType: 'water', elementType: 'geometry', stylers: [{ color: '#a2d2ff' }] },
          { featureType: 'road', elementType: 'geometry', stylers: [{ color: '#ffffff' }] },
          { featureType: 'poi', elementType: 'geometry.fill', stylers: [{ color: '#cfe2f3' }] },
        ]
      });
  ...
  useEffect(() => {
    ...
    events.forEach((event) => {
      ...
      const getMarkerIcon = (type) => {
        ...
        const fillColor = colors[type] || '#6b7280';
  ...
  return (
    <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border-4 border-gradient-to-r from-pink-200 to-blue-200">
      ...
      <div className="relative">
        <div ref={mapRef} className="h-96 bg-gradient-to-br from-blue-100 via-purple-50 to-pink-100">
          ...
          <div className="absolute top-4 left-4 right-4 bg-white rounded-2xl shadow-2xl border-4 border-gradient-to-r from-yellow-300 to-pink-300 z-10 max-w-md mx-auto">
            <div className={`${getEventTypeStyle(selectedEvent.type)} p-4 text-white rounded-t-xl relative overflow-hidden`}>
              ...
              <div className="relative flex items-center justify-between">
                ...
                <button onClick={closeEventDetails} className="w-8 h-8 bg-white/20 hover:bg-white/30 rounded-full flex items-center justify-center transition-all duration-200 hover:scale-110">
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>
            <div className="p-4 bg-gradient-to-br from-white to-blue-50">
              ...
              <div className="mt-4 flex gap-2">
                ...
                <button onClick={closeEventDetails} className="px-4 py-3 bg-gray-100 hover:bg-gray-200 rounded-xl font-medium transition-all duration-200 hover:scale-105">
                  Close
                </button>
              </div>
            </div>
      ...
      <div className="p-6 bg-gradient-to-r from-blue-50 via-purple-50 to-pink-50 border-t-4 border-gradient-to-r from-blue-300 to-pink-300">
        <h4 className="text-lg font-bold text-gray-800 mb-4 text-center">🎨 Event Type Legend 🎨</h4>
